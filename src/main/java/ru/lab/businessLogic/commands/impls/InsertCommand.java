...
public class InsertCommand extends AbstractCommand {
    ...
    public InsertCommand(Set<CommandArgs> commandArgsSet,
                         CollectionWrapper<Movie> collectionWrapper,
                         ObjectMapper xmlMapper) {
        super(CommandType.MIXED, "insert_at", commandArgsSet, "insert element at position ");
    ...
    public CommandResult execute(Query query) throws Exception {
        ...
        try {
            ...
            long position = Long.parseLong(args.get(CommandArgs.SIMPLE_ARGUMENT));
            collectionWrapper.insertAt(position, movie);
            logger.info("Command insert_at successfully executed!");
            return new CommandResult(
                    "movie " + movie.getName() + " inserted into " + position,
